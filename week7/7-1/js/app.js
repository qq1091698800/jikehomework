...
$(document).ready(function () {
    ...
    if (c.indexOf('theme=(') >= 0) {
        ...
        if (lastSkinId != '')
            lastBgImgUrl = 'images/skin/' + lastSkinId + '.png';
        ChangeBG();
    }
...
function ChangeBG() {
    if (lastSkinId == '') {
        ...
        $('#mainlogo').attr('src', 'images/bd_logo1.png');
        $('body').css({ 'background': 'none' });
        $('#skinprevtop').hide();
        lastBgImgUrl='images/skin-prev-0.png';
        $('#skinprev').attr('src',lastBgImgUrl);
        ...
        $('#mainlogo').attr('src', 'images/bd_logo2.png');
...
function PlaceSkinImgs(skintype, skingroupindex, isini) {
    ...
    for (var i = 1; i <= 12; i++) {
        $('.picsul>li').eq(i - 1).find('img').attr('src', 'images/skin/'+(skins[skintype-1].groups[skingroupindex]%10+1)+'-'+i+'.png');
    }
